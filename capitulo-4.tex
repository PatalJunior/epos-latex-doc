...
The work was organised into three iterations, each carried out in its own
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		node distance=8mm,
		box/.style={draw, rounded corners=2pt, align=center, thick, minimum height=8mm, minimum width=28mm},
		...
		\node[stage, below=of client]
			(resource)
			{\textbf{BookingResource} \\\footnotesize \texttt{@Inject BookingService}};
		\node[stage, below=of resource]
			(svc)
			{\textbf{BookingService} \\\footnotesize business logic};
		\node[stage, below=of svc, fill=red!12] (ex) {Exception thrown};
		\node[stage, below=of ex]
			(dispatch)
			{JAX-RS exception dispatch \\\footnotesize matches type};

		\node[mapper, below=10mm of dispatch, xshift=-58mm]
			(m1)
			{BusinessValidation\\Mapper};
		...
		\node[outbox, below=10mm of m2, xshift=20mm]
			(resp)
			{Response.builder() \\\footnotesize \texttt{\{ "error": "...", "details": [...] \}}};
		\node[stage, below=of resp]
			(cliback)
			{HTTP client \\\footnotesize sees structured JSON, never a stack trace};

		\draw[arrow]
			(client) --
			node[right, font=\scriptsize] {POST /api/v1/bookings}
			(resource);
...
Solid arrows denote control flow inside the pipeline; dashed arrows denote
artifact or data access. The \texttt{http-diff-job} runs a three-way
comparison between the \texttt{HEAD} build and two baselines, the immediate
parent commit and a fixed reference commit, all pointed at the same database
fixture.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		every node/.style={font=\small},
		box/.style={draw, rounded corners=2pt, minimum height=10mm, minimum width=28mm, align=center, thick},
		smallbox/.style={box, minimum width=22mm, minimum height=8mm, font=\footnotesize},
		arrow/.style={-{Stealth[length=2mm]}, thick}
	]
		\node[box, fill=lgray] (push) {Developer push \\ on a branch};
		\node[box, right=18mm of push, fill=dkblue!10]
			(gitlab)
			{GitLab CI server \\ (self-managed)};
		\node[box, right=18mm of gitlab, fill=dkblue!10]
			(runner)
			{GitLab Runner \\ (\texttt{ubi-runner-.31})};

		\node[smallbox, below=12mm of runner, fill=dkgreen!10]
			(build)
			{\texttt{build-job} \\ \scriptsize mvn package};
		\node[smallbox, below=of build, fill=dkgreen!10]
			(test)
			{\texttt{integration-test-job}};
		\node[smallbox, below=of test, fill=dkgreen!10]
			(http)
			{\texttt{http-diff-job}};

		\node[box, left=24mm of test, fill=mauve!10] (head) {HEAD WAR};
		\node[box, below=of head, fill=mauve!10]
			(brecent)
			{Baseline WAR \\ recent \\ (\texttt{HEAD\textasciitilde 1})};
		\node[box, below=of brecent, fill=mauve!10]
			(bfixed)
			{Baseline WAR \\ fixed \\ (\texttt{\$DIFF\_FIXED\_BASELINE})};

		\node[box, below=10mm of bfixed, fill=dark!10]
			(db)
			{PostgreSQL \\ (\texttt{192.168.168.30}) \\ \scriptsize J or F fixture};

		\draw[arrow] (push) -- (gitlab);
		\draw[arrow] (gitlab) -- (runner);
		\draw[arrow] (runner) -- (build);
		\draw[arrow] (build) -- (test);
		\draw[arrow] (test) -- (http);
...
A push by the developer triggers a pipeline on the runner tagged \texttt{ubi-runner-.31},
which goes through three jobs across three stages: \texttt{build-job}
produces the \texttt{HEAD} \ac{WAR}, \texttt{integration-test-job} runs the
regular JUnit integration tests, and \texttt{http-diff-job} reconstructs
both baseline \acp{WAR} via Git worktrees, boots all three Payara Micro
instances on different ports, replays \texttt{http-diff-cases.json} against
each and emits per-baseline diff reports. All reports are picked up by GitLab
as both JUnit results and raw artifacts.

The two prototypes traded a longer time-to-result for a sharper separation of
concerns between \ac{CI}/\ac{CD} infrastructure and Jakarta EE feature usage;
the next chapter applies the resulting framework to the production \ac{API}.